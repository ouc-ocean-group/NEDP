...
def basic_cell(lstm_cell_num,keep_prob,mode,n_seqs,n_steps,d):
	if mode == 'lstm':
		lstm = tf.nn.rnn_cell.LSTMCell(lstm_cell_num,initializer=tf.orthogonal_initializer,forget_bias=0.0)
	elif mode == 'rnn':
		lstm = tf.nn.rnn_cell.BasicRNNCell(lstm_cell_num)
	else:
		raise Exception('Unkown mode:{},only support rnn and lstm mode'.format(mode))

	return tf.compat.v1.nn.rnn_cell.DropoutWrapper(lstm,output_keep_prob=keep_prob)

def hidden_layer(lstm_cell_num,lstm_layer_num,n_seqs,keep_prob,mode,n_steps,d):

	multi_lstm = tf.nn.rnn_cell.MultiRNNCell([basic_cell(lstm_cell_num,keep_prob,mode,n_seqs,n_steps,d) for _ in range(lstm_layer_num)])
...
class LSTM:
    def __init__(self,class_num,n_steps,n_seqs,lstm_cell_num,lstm_layer_num,learning_rate,grad_clip,mode,d,t,alpha):
        ...
        #lstm layer
        lstm = tf.contrib.cudnn_rnn.CudnnLSTM(num_layers=lstm_layer_num,num_units=lstm_cell_num,dropout=0.5)
        ...
        lstm.build(x_input.get_shape())
        output,state = lstm(x_input, training=True)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_integer('sequences',10,'how many sequences in one batch')
    ...
    for i in range(FLAGS.epoches):
        ...
        for j in range(FLAGS.gen_epoches):
            ...
            for x,y in get_batch(encode_arr_input,FLAGS.sequences,FLAGS.timesteps):
                ...
                feed_dict = {lstm.input:x,lstm.targets:y,lstm.keep_prob:FLAGS.keep_prob}
                batch_loss,new_state,_ = sess.run([lstm.loss,lstm.final_state,lstm.optimizer],feed_dict=feed_dict)
                count = count + 1
                if count % 500 == 0:
                    print(i,j,count, batch_loss)
